...
function select_poslanec(mysqli $mysqli, string $email): array
{
    if ((!empty($email) || !empty($user_id)) && !$mysqli->connect_errno) {
        $email = filter_var($email, FILTER_SANITIZE_EMAIL);
...
function select_admin(mysqli $mysqli, string $email): array
{
    if ((!empty($email) || !empty($user_id)) && !$mysqli->connect_errno) {
        $email = filter_var($email, FILTER_SANITIZE_EMAIL);
...
function select_user(mysqli_stmt $stmt, string $email): array
{
    $stmt->bind_param('s', $email);
    $stmt->execute();
    $result = $stmt->get_result();
...
function update_admin(mysqli $mysqli, array $admin): int
{
    if (!$mysqli->connect_errno) {
        ...
        $update_result = update_osobne_udaje($mysqli, $admin['email'], $admin['first_name'], $admin['last_name'], $admin['address']);
        ...
        $stmt->bind_param('isi', empty($admin['id_previerka']) ? $old_data['id_previerka'] : $admin['id_previerka'],
            empty($admin['heslo']) ? $old_data['heslo'] : $pswd,
            $old_data['id']);
        $stmt->execute();
        if ($stmt->affected_rows > 0) return SUCCESS;
    }
...
function update_poslanec(mysqli $mysqli, array $poslanec): int
{
    if (!$mysqli->connect_errno) {
        ...
        $update_result = update_osobne_udaje($mysqli, $poslanec['email'], $poslanec['first_name'], $poslanec['last_name'], $poslanec['address']);
        if ($update_result == ERROR_USER_EXISTS) return ERROR_USER_EXISTS;
        ...
        $stmt->bind_param('iisssi', empty($poslanec['id_klub']) ? $old_data['id_klub'] : $poslanec['id_klub'],
            ...
            empty($poslanec['heslo']) ? $old_data['heslo'] : $pswd,
            $old_data['id']);
        $stmt->execute();
        if ($stmt->affected_rows > 0) return SUCCESS;
    }
